CREATE DATABASE "iot-on-earth"
    WITH
    OWNER = "datacanvasAdmin"
    ENCODING = 'UTF8'
...
COMMENT ON SCHEMA "iot-on-earth-public"
    IS 'standard public schema';


CREATE TABLE IF NOT EXISTS "iot-on-earth-public".users
(
    user_id integer NOT NULL DEFAULT nextval('"iot-on-earth-public".users_user_id_seq'::regclass),
...
(
    ...
    CONSTRAINT projects_user_id_fkey FOREIGN KEY (user_id)
        REFERENCES "iot-on-earth-public".users (user_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
)
...
(
    ...
    CONSTRAINT devices_project_id_fkey FOREIGN KEY (project_id)
        REFERENCES "iot-on-earth-public".projects (project_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
)
...
(
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT datatables_pkey PRIMARY KEY (tbl_id),
...
CREATE TABLE IF NOT EXISTS "iot-on-earth-public".datatypes
(
    type_id integer NOT NULL DEFAULT nextval('"iot-on-earth-public".columndatatypes_type_id_seq'::regclass),
    type_name character varying(10) COLLATE pg_catalog."default" NOT NULL,
    CONSTRAINT columndatatypes_pkey PRIMARY KEY (type_id)
)

CREATE TABLE IF NOT EXISTS "iot-on-earth-public"."constraint"
(
    constraint_id integer NOT NULL DEFAULT nextval('"iot-on-earth-public".columnconstraints_constraint_id_seq'::regclass),
...
(
    ...
    max_length integer,
    CONSTRAINT columns_pkey PRIMARY KEY (clm_id),
    CONSTRAINT columns_data_type_fkey FOREIGN KEY (data_type)
        REFERENCES "iot-on-earth-public".datatypes (type_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    CONSTRAINT columns_tbl_id_fkey FOREIGN KEY (tbl_id)
        REFERENCES "iot-on-earth-public".datatables (tbl_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
)
...
(
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT constraintsofcolumn_pkey PRIMARY KEY (id),
    CONSTRAINT columnconstraint_clm_id_fkey FOREIGN KEY (clm_id)
        REFERENCES "iot-on-earth-public".columns (clm_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT constraintsofcolumn_constraint_id_fkey FOREIGN KEY (constraint_id)
        REFERENCES "iot-on-earth-public"."constraint" (constraint_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT
)
...
(
    ...
    CONSTRAINT charts_widget_fkey FOREIGN KEY (widget_id)
        REFERENCES "iot-on-earth-public".widgets (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
...
(
    ...
    CONSTRAINT chartseries_chart_id_fkey FOREIGN KEY (chart_id)
        REFERENCES "iot-on-earth-public".charts (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    ...
    CONSTRAINT chartseries_device_id_fkey FOREIGN KEY (device_id)
        REFERENCES "iot-on-earth-public".devices (device_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
        NOT VALID
)
...
(
    ...
    device_id integer,
    CONSTRAINT toggles_pkey PRIMARY KEY (id),
